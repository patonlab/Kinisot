...
def test_isotope_effect(
    ...
    (rpfrs, zpe_val, exc_val, trpf_val, kie_val, corr_kie_val, tunn_corr_val, freq_fac) = run_kie(
        reactants, ts, prd, iso, temperature, scaling
    )
...
def test_ts_imaginary_frequency_detected():
    ...
    rpfrs, *_ = run_kie(["gaussian/claisen_gs.out"], ["gaussian/claisen_ts.out"], None, ["5", "5"], 393.0, 0.961)
