...
def test_multiple_reactant_files(run):
    rc, tmp_path = run(['--rct', datapath('gaussian/dienophile.out'), '--rct', datapath('gaussian/diene.out'),
                        '--ts', datapath('gaussian/DATS.out'), '--iso', '0', '--iso', '6', '--iso', '15', '-s', '0.963'])
    assert rc == 0
    out = read(tmp_path)
...
def test_python_m_kinisot(tmp_path):
    env = dict(os.environ)
    env['PYTHONPATH'] = os.pathsep.join(p for p in [os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                                    env.get('PYTHONPATH', '')] if p)
    proc = subprocess.run([sys.executable, '-m', 'kinisot'] + CLAISEN, cwd=str(tmp_path), env=env,
