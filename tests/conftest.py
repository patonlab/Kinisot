...
def write_gaussian_like(
    path,
    atomic_numbers,
    masses,
    hessian,
    rotational="12.49 1.87 1.64",
    level="RB3LYP",
    basis="6-31G(d)",
    nimag=0,
    separator="\\",
    ...
    if archive:
        ...
        body = (
            "1\\1\\GINC-TEST\\Freq\\%s\\%s\\C1H1\\USER\\01-Jan-2026\\0\\\\# freq\\\\title\\\\0,1\\C,0.,0.,0."
            "\\\\Version=ES64L-G16RevC.01\\NImag=%d\\\\"
            % (level, basis, nimag)
            + ",".join("%.12e" % x for x in tri)
            + "\\\\0.,0.,0.\\\\@"
        )
