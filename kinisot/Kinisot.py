...
def get_frequency_scaling(files, log):
    ...
    if len(set(l_o_t)) > 1:
        log.Writeonlyfile(
            "\nWARNING: found different levels of theory across input files: "
            + " / ".join(l_o_t)
        )
    else:
        level = l_o_t[0]
    ...
    if freq_scale_factor == 1.00:
        log.Write(
            "\n  Unable to find vibrational scaling factor for "
            + level
            + "; using value of 1.0"
        )
...
class calc_rpfr:
    ...
    def __init__(
        ...
        for i, file in enumerate(files):
            ...
            freqs = [
                np.copysign(np.sqrt(np.abs(freq)), freq) * freq_scale_factor
                for freq in eigs
            ]
            ...
            for freq in freqs[trans_rot_modes:]:
                self.frequency_wn.append(freq)
...
def compute_isotope_effect(
    rct, ts, prd, label, temperature=298.15, freq_scale_factor=1.0, freq_cutoff=50.0
):
    ...
    if ts is not None:
        parabolic_tunn_corr = (
            freq_fac
            * math.sin(0.5 * tofreq * KIE[3].im_frequency_wn)
            / math.sin(0.5 * tofreq * KIE[2].im_frequency_wn)
        )
    else:
...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        required=True,
        help="comma-separated atom number(s) to substitute with the heavier isotope; use 0 for a file with no substitution. Repeat the flag once per input file (a single value is applied to all files)",
    )
    parser.add_argument(
        "--cutoff",
        dest="freq_cutoff",
    ...
    if options.ts is not None:
        files = options.rct + options.ts
    else:
        files = options.rct + options.prd
    ...
    if len(files) != len(options.label):
        parser.error(
            "for multiple reactants you need to specify --iso labels for each of the {} input files".format(
                len(files)
            )
        )
    ...
    with Logger("Kinisot", "dat", "output") as log:
        ...
        if options.ts is not None:
            log.Write("{:10.1f}".format(KIE[2].im_frequency_wn))
        log.Write(
            "\no "
            + (
                options.rct[0].split(".")[0]
                + ": iso @ "
                + " / ".join(options.label[0 : len(options.rct)])
            ).ljust(47)
        )
        log.Write(
            "           {:10.3e} {:10.3e} {:10.3e}".format(
                np.e ** (KIE[0].ZPE - KIE[1].ZPE),
                np.e ** (KIE[0].EXC - KIE[1].EXC),
                np.e ** (KIE[2].PF - KIE[3].PF),
            )
        )
        if options.ts is not None:
            log.Write(
                "\no "
                + (
                    options.ts[0].split(".")[0]
                    + ": iso @ "
                    + " / ".join(options.label[len(options.rct) :])
                ).ljust(47)
            )
        else:
            log.Write(
        ...
        if options.ts is not None:
            log.Write(
                "{:10.1f} {:10.3e} {:10.3e} {:10.3e}".format(
                    KIE[3].im_frequency_wn,
                    np.e ** (KIE[2].ZPE - KIE[3].ZPE),
                    np.e ** (KIE[2].EXC - KIE[3].EXC),
                    np.e ** (KIE[0].PF - KIE[1].PF),
                )
            )
        else:
            log.Write(
