...
def substitute_isotopes(mass_list, iso):
   ...
   for atom in iso.split(','):
      try:
         ...
         raise ValueError("Invalid atom number {!r} in isotope specification {!r}".format(atom, iso))
      if i == -1:
         continue
      if i < -1 or i >= len(mass_list):
         raise ValueError("Atom number {} in isotope specification {!r} is out of range: "
