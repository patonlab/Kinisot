...
5. `tests/test_crossval.py`: mass-weighted Hessian from
   `goodvibes.io.parse_hessian` equals `read_hess` to 1e-12; optional
...
## Phase 5 — GoodVibes integration and ORCA support (v2.3.0)
...
1. **Dependency**: `goodvibes>=4.4` in `pyproject.toml` and the conda
   recipe.
2. **Upstream PR to GoodVibes (small)**: `FUNCTIONAL_ALIASES` for
...
## Phase 7 — Isotopes, external modes, tunnelling (v2.4.0)

Prioritized by user demand; each item is an independent PR with its own
docs and example.
...
## Phase 8 — ASE and machine-learned potentials (v2.5.0)
...
## Phase 9 — Experimental validation suite (v2.5.x, after Phase 7)
...
| 9 | Experimental validation suite (Singleton KIEs) | v2.5.x | needs new QC calculations |
